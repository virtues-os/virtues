...
async fn main() -> Result<()> {
    ...
    let db = connect_from_env("virtues-action-ios_healthkit").await?;

    // The iOS app posts `{source, stream, deviceId, records: [..], timestamp,
    // checkpoint}`. Accept either that shape or a bare array (for manual /
    // cron invocations).
    let records = input
        .payload
        .as_ref()
        ...
        .ok_or_else(|| anyhow::anyhow!("ios_healthkit requires `records` array in payload"))?;
