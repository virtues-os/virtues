...
pub async fn write_app_events(db: &SqlitePool, events: &[Value]) -> Result<usize> {
    if events.is_empty() {
        return Ok(0);
    }
...
pub async fn write_browser_history(db: &SqlitePool, visits: &[Value]) -> Result<usize> {
...
pub async fn write_imessages(db: &SqlitePool, messages: &[Value]) -> Result<usize> {
    let mut pending: Vec<(
        String,
        String,
        String,
