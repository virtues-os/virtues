...
use virtues_action_helpers::ios::{stream_id_or_new, EVENTKIT_STREAM_TABLE, IOS_PROVIDER};
...
pub async fn write_events(db: &SqlitePool, records: &[Value]) -> Result<usize> {
    if records.is_empty() {
        return Ok(0);
    }
...
async fn flush_events(db: &SqlitePool, records: &[CalendarRow]) -> Result<usize> {
