...
fn shape_one(msg: &Value, user_email: &str) -> Option<EmailRow> {
    ...
    let label_ids: Vec<String> = msg
        ...
        .unwrap_or_default();
    let is_read = if label_ids.iter().any(|l| l == "UNREAD") { 0 } else { 1 };
    let is_starred = if label_ids.iter().any(|l| l == "STARRED") { 1 } else { 0 };
    ...
    let has_attachments = if has_attachments(payload) { 1 } else { 0 };
...
async fn flush(db: &SqlitePool, records: &[EmailRow]) -> Result<usize> {
    if records.is_empty() {
        return Ok(0);
    }
