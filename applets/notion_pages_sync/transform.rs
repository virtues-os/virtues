//! Notion pages → `data_content_document` transform.
//!
...
pub async fn write_pages(db: &PgPool, pages: &[Value]) -> Result<usize> {
...
async fn flush(db: &PgPool, records: &[DocRow]) -> Result<usize> {
    if records.is_empty() {
        return Ok(0);
    }
