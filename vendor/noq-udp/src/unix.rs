    ...
    pub(crate) fn max_gso_segments(socket: &impl AsRawFd) -> usize {
        ...
        match set_socket_option(socket, libc::SOL_UDP, libc::UDP_SEGMENT, GSO_SIZE) {
            Ok(()) => {
                ...
                // VIRTUES PATCH (temporary — drop when quinn PR #2653 ships the
                // EIO→resend-as-individual-datagrams fallback and we bump iroh):
                // some NICs (the Jetson Orin tegra onboard wifi) PASS this probe
                // but then fail EVERY GSO `sendmsg` with EIO, and the runtime
                // "halting segmentation offload" store does not propagate to
                // quinn-proto's per-connection batching — so GSO keeps being used
                // and the QUIC path stalls (no data over direct IPv6). A personal
                // box/client moves little data, so GSO's syscall-batching win is
                // irrelevant; reporting 1 keeps GSO off entirely and is
                // NIC-agnostic (no probe/env/allowlist). The probe above still
                // runs so no upstream symbols go unused.
