	...
	<div class="max-w-7xl">
		...
		<div class="mb-6">
			<div class="flex items-center gap-2 mb-2">
				...
				<h1 class="text-3xl font-serif font-medium text-foreground">Trash</h1>
			</div>
			<p class="text-foreground-muted">Items in Trash are automatically deleted after 30 days</p>
		</div>
		...
		<div class="border border-border rounded-lg overflow-hidden">
			...
			{:else if trashFiles.length === 0}
				<div class="p-12 text-center">
					<Icon icon="ri:delete-bin-line" class="text-6xl text-foreground-subtle mb-4"/>
...
<Modal open={!!fileToRestore} onClose={() => (fileToRestore = null)} title="Restore {fileToRestore?.is_folder ? 'Folder' : 'File'}?" width="sm">
	<p class="text-foreground-muted">
	...
	{#snippet footer()}
		<button class="modal-btn modal-btn-secondary" onclick={() => (fileToRestore = null)}>
			Cancel
		</button>
		<button
...
<Modal open={!!fileToPurge} onClose={() => (fileToPurge = null)} title="Delete Forever?" width="sm">
	<p class="text-foreground-muted">
	...
	{#snippet footer()}
		<button class="modal-btn modal-btn-secondary" onclick={() => (fileToPurge = null)}>
			Cancel
		</button>
		<button
...
<Modal open={showEmptyTrashModal} onClose={() => (showEmptyTrashModal = false)} title="Empty Trash?" width="sm">
	<p class="text-foreground-muted">
		All {trashFiles.length} {trashFiles.length === 1 ? 'item' : 'items'} in Trash will be permanently deleted.
	</p>
	<p class="text-red-500 font-medium mt-2">This action cannot be undone.</p>
	{#snippet footer()}
		<button class="modal-btn modal-btn-secondary" onclick={() => (showEmptyTrashModal = false)}>
			Cancel
		</button>
		<button
			class="modal-btn bg-red-500 text-white hover:bg-red-600 disabled:opacity-50"
