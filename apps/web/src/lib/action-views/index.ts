 ...
 *
 * View-runtime actions are self-contained under `actions/<name>/`. Their UI
 * lives next to the manifest at `actions/<name>/ui/`:
 *
 *   actions/<name>/manifest.toml          (declares config.view.name)
 *   actions/<name>/ui/Card.svelte         (optional — overrides TemplateCard)
 *   actions/<name>/ui/Detail.svelte       (optional — overrides ActionDetailView)
 *   actions/<name>/ui/Output.svelte       (future — for run-output rendering)
 *
 ...
 *
 * The folder name under `actions/` is the lookup key. It does NOT have to
