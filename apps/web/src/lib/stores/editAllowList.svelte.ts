...
export const activePageStore = {
	...
	get boundPageTitle() {
		const pages = editAllowListStore.items.filter((i) => i.type === 'page');
		return pages.length > 0 ? pages[0].title : null;
	},
	get isBound() {
		return editAllowListStore.items.some((i) => i.type === 'page');
	},
	bind(pageId: string, pageTitle: string, yjsDoc: YjsDocument) {
		// For backward compat, clear existing pages and add new one
		const existingPages = editAllowListStore.items.filter((i) => i.type === 'page');
	...
	getYjsDoc(): YjsDocument | null {
		const pages = editAllowListStore.items.filter((i) => i.type === 'page');
		return pages.length > 0 ? (pages[0].yjsDoc ?? null) : null;
	}
};
