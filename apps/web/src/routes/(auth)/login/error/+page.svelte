<script lang="ts">
	...
	const errorMessages: Record<string, { title: string; message: string }> = {
		...
		AccessDenied: {
			title: "Access Denied",
			message:
				"This email is not authorized to sign in to this instance.",
		},
		Verification: {
			title: "Link Expired",
			message:
...
<div class="w-full max-w-sm text-center">
	...
	<a
		href="/login"
		class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg bg-foreground text-background font-medium hover:bg-foreground/90 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-colors"
	>
