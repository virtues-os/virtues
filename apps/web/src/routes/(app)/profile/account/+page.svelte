<script lang="ts">
	...
	let currentTheme = $state<Theme>("scriptorium");
	...
	onMount(async () => {
		...
		fallbackView = windowTabs.fallbackPreference;
		await loadProfile();
	...
	async function handleFallbackChange(newFallback: FallbackView) {
		...
		windowTabs.setFallbackPreference(newFallback);

		try {
			// Fetch current profile to preserve other ui_preferences
			...
			<div class="space-y-6 mb-6">
				...
				<div class="bg-surface border border-border rounded-lg p-6">
					...
					<p class="text-sm text-foreground-muted mb-4">
						What to show when all tabs are closed
					</p>
