			...
			{#each Object.entries(groupedOntologies) as [domain, ontologies]}
				<div class="mb-8">
					...
					{#if ontologies.length === 0}
						<div
							class="border border-dashed border-border rounded-lg p-8 text-center bg-surface-elevated/50"
						>
					...
					{:else}
						...
						>
							{#each ontologies as ontology}
								<div
									class="p-4 bg-surface border border-border rounded-lg hover:border-border-subtle transition-all duration-200"
								>
									<!-- Header -->
									<div class="mb-3">
										<h3
											class="font-medium text-foreground text-sm mb-1"
										>
											{ontology.name}
										</h3>
									...
									{#if ontology.sample_record}
										...
										>
											<button
												...
												class="w-full text-left flex items-center justify-between text-xs text-foreground-muted hover:text-foreground transition-colors"
											>
											...
											{#if expandedSamples.has(ontology.name)}
												<div
													class="mt-2 p-2 bg-surface-elevated rounded border border-border max-h-48 overflow-auto"
												>
													<pre
														class="text-xs text-foreground-muted whitespace-pre-wrap break-words font-mono">{JSON.stringify(
									...
									{:else if ontology.record_count === 0}
										...
										>
											<p
												class="text-xs text-foreground-subtle italic"
											>
												No data yet
											</p>
										</div>
