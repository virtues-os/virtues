import type { Job, ActivityMetrics } from '$lib/api/client';
...
export const load: PageLoad = async ({ url, fetch }) => {
	try {
		...
		// Fetch jobs, sources, and metrics in parallel
		const [jobsRes, sourcesRes, metricsRes] = await Promise.all([
		...
		const metrics: ActivityMetrics | null = metricsRes.ok ? await metricsRes.json() : null;
