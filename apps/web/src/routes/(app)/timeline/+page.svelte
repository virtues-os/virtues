<script lang="ts">
	...
	$: windowStart = new Date(
		`${data.dayView.date}T${selectedHour.toString().padStart(2, "0")}:00:00+01:00`,
	); // Rome timezone
	...
	function getBlockStyle(
		...
		const clampedStart = new Date(
			Math.max(chunkStart.getTime(), windowStart.getTime()),
		);
		const clampedEnd = new Date(
			Math.min(chunkEnd.getTime(), windowEnd.getTime()),
		);
		...
		const startPercent =
			...
			100;
		const endPercent =
			((clampedEnd.getTime() - windowStart.getTime()) /
				(24 * 60 * 60 * 1000)) *
			100;
	...
	// Summary stats
	$: locationCount = data.dayView.chunks.filter(
		(c) => c.type === "location",
	).length;
	$: transitCount = data.dayView.chunks.filter(
		(c) => c.type === "transit",
	).length;
...
<Page>
	<div class="max-w-7xl">
		...
		<div class="mb-8">
			...
			<div class="flex items-center gap-4 mb-4">
				<div class="flex items-center gap-2">
					<label for="date" class="text-sm text-foreground-muted"
						>Date:</label
					>
					<Input
						id="date"
				...
				<div class="flex items-center gap-2">
					<label for="hour" class="text-sm text-foreground-muted"
						>Start hour:</label
					>
					<select
						id="hour"
		...
		{#if data.error}
			<!-- Error State -->
			<div
				class="border-2 border-error rounded-lg p-8 text-center bg-error-subtle"
			>
				<iconify-icon
					icon="ri:error-warning-line"
					class="text-4xl text-error mb-2"
				></iconify-icon>
		...
		{:else if data.dayView.chunks.length === 0}
			...
			>
				<iconify-icon
					...
					class="text-4xl text-foreground-subtle mb-2"
				></iconify-icon>
				<h3 class="text-lg font-medium text-foreground mb-1">
					No timeline data for this day
				</h3>
				<p class="text-foreground-muted mb-3">
					Try selecting a different date or run the seed script
				</p>
				<code
					class="text-sm text-foreground-muted px-3 py-2 rounded bg-surface-elevated"
				>
		...
		{:else}
			...
			<div class="mb-6">
				...
				<div class="flex flex-wrap gap-4 text-sm text-foreground-muted">
					...
					<span>·</span>
					<span
						>{transitCount} transit{transitCount !== 1
							? "s"
							: ""}</span
					>
					<span>·</span>
					<span
						>{formatDuration(data.dayView.total_location_minutes)} at
						places</span
					>
					<span>·</span>
					<span
						>{formatDuration(data.dayView.total_transit_minutes)} moving</span
					>
				</div>
			</div>
			...
			<!-- Horizontal Timeline -->
			<div
				class="relative bg-surface border border-border rounded-lg p-6"
			>
				...
				<div class="relative">
					...
					<div class="relative h-16">
						{#each visibleChunks as chunk, index}
							...
							{#if style}
								<button
									class="absolute top-2 bottom-2 {getChunkClasses(
										chunk,
									)} border rounded cursor-pointer hover:brightness-95 transition-all {selectedChunkIndex ===
									index
			...
			{#if selectedChunkIndex !== null && visibleChunks[selectedChunkIndex]}
				...
				<div
					class="mt-4 p-4 bg-surface-elevated border border-border rounded-lg"
				>
					{#if chunk.type === "location"}
						<div class="flex items-start gap-3">
							<iconify-icon
								icon={chunk.is_known_place
									? "ri:map-pin-fill"
									: "ri:map-pin-line"}
							...
							<div class="flex-1">
								...
								<div class="text-sm text-foreground-muted mb-3">
									{formatTime(new Date(chunk.start_time))} - {formatTime(
										new Date(chunk.end_time),
									)}
								</div>
									...
									>
										{#each chunk.messages as msg}
											...
											>
												<iconify-icon
													icon="ri:chat-1-line"
													class="text-orange-500 mt-0.5"
												></iconify-icon>
												<span
													class="text-foreground-muted"
													>{msg.from_name ||
														msg.channel}: {msg.body_preview}</span
												>
											</div>
										{/each}
										{#each chunk.transcripts as transcript}
											...
											>
												...
												<span
													class="text-foreground-muted"
													>{transcript.transcript_preview}</span
												>
											</div>
										{/each}
					...
					{:else if chunk.type === "transit"}
						<div class="flex items-start gap-3">
							<iconify-icon
								icon="ri:walk-line"
								class="text-foreground-subtle text-xl mt-0.5"
							></iconify-icon>
							<div class="flex-1">
								<div
									class="flex items-baseline justify-between gap-2 mb-1"
								...
								<div class="text-sm text-foreground-muted">
									...
									<span class="ml-2"
										>{chunk.distance_km.toFixed(1)} km · {chunk.avg_speed_kmh.toFixed(
											0,
										)} km/h</span
									>
								</div>
								{#if chunk.from_place || chunk.to_place}
									<div
										class="text-sm text-foreground-subtle mt-1"
									>
						...
						<div class="flex items-start gap-3">
							...
							<div class="flex-1">
								...
								<div class="text-sm text-foreground-muted">
									...
									<span class="ml-2 text-foreground-subtle"
										>({getMissingReason(
											chunk.likely_reason,
										)})</span
									>
			...
			<div class="flex flex-wrap gap-4 text-xs mt-6">
				<div class="flex items-center gap-2">
					<div
						class="w-4 h-4 bg-blue-400 border border-blue-500 rounded"
					></div>
					<span class="text-foreground-muted">Location</span>
				</div>
				<div class="flex items-center gap-2">
					<div
						class="w-4 h-4 bg-neutral-300 border border-neutral-400 rounded"
					></div>
					<span class="text-foreground-muted">Transit</span>
				</div>
				<div class="flex items-center gap-2">
					<div
						class="w-4 h-4 bg-surface-elevated border-2 border-dashed border-border rounded"
					></div>
