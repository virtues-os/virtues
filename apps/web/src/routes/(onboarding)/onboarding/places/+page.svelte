<script lang="ts">
	...
	let homeAddress = $state(homeLocation?.formatted_address || homeLocation?.address || "");
	let homePlace = $state<Place | null>(homeLocation ? toPlace(homeLocation) : null);
...
<div class="markdown w-full max-w-xl mx-auto">
	<header>
		...
		<p class="mt-2 text-foreground-muted">
			Understanding where you live helps your Personal AI provide contextual
			insights about your daily life and travels.
		</p>
	</header>
	...
	<section class="mt-8">
		<h2>Home Address</h2>
		<p class="mt-2 text-foreground-muted">
			This is used to understand your local context and will never be shared.
		</p>
		<div class="flex flex-col gap-4 mt-4">
			<label class="flex flex-col gap-2">
				<span class="text-sm text-foreground-subtle">Your home address</span>
				<PlacesAutocomplete
					value={homeAddress}
	...
	<section class="mt-8">
		...
		<div class="flex flex-col gap-4 mt-4">
			...
			<button
				type="button"
				onclick={addPlace}
				disabled={!newPlaceLabel.trim() || !newPlaceAddress.trim()}
