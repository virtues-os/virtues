<script lang="ts">
	...
	// Features list
	const FEATURES = [
		{ key: "autobiography", label: "Automatic life logging & autobiography" },
		...
		{ key: "health", label: "Health tracking (sleep, heart rate, nutrition)" },
		{ key: "timeline", label: "Unified timeline of your life" }
	];

	// Form state
	let vision = $state(initialData?.profile?.technology_vision || "");
	let painPointPrimary = $state(initialData?.profile?.pain_point_primary || "");
	let painPointSecondary = $state(initialData?.profile?.pain_point_secondary || "");
	let excitedFeatures = $state<string[]>(initialData?.profile?.excited_features || []);
	...
	$effect(() => {
		const isValid = !!vision.trim() && !!painPointPrimary && excitedFeatures.length > 0;
...
<div class="markdown w-full max-w-xl mx-auto">
	<header>
		...
		<p class="text-foreground-subtle mt-2">
			Help us understand your relationship with technology and what you hope to get from Virtues.
		</p>
	</header>
	...
	<section class="mt-8">
		...
		<div class="mb-4 p-4 bg-surface-elevated rounded-lg border border-border">
			<p class="text-sm text-foreground-subtle mb-2 font-medium">Example responses:</p>
			<p class="text-sm text-foreground-subtle italic mb-3">
				"I'm 25, moved to a new city for work, and feel socially isolated. I want my AI to notify me when I've been home too long, proactively search for local events and sports leagues, and message friends to check availability — helping me get outside and develop real relationships, not drown in screens and entertainment."
			</p>
			<p class="text-sm text-foreground-subtle italic">
				"I've tried every todo app and they all feel soulless. I want an AI that knows I value deep work and family time, blocks my calendar accordingly, and pushes back when I overcommit — not one that just helps me do more faster."
			</p>
		</div>
	...
	<section class="mt-10">
		...
		<p class="mb-4">
			Select your <span class="font-medium">primary</span> reason for seeking out Virtues, and optionally a secondary one.
		</p>
		...
		<div class="flex flex-col gap-1">
			{#each PAIN_POINTS as point}
				...
				>
					<div class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 {isPrimary
						...
						{:else if isSecondary}
							<span class="text-[9px] text-surface font-bold">2</span>
						{/if}
					</div>
					<span class="text-sm {isSelected ? 'text-foreground font-medium' : 'text-foreground-subtle'}">
						{point.text}
					</span>
				</button>
			{/each}
	...
	<section class="mt-10">
		...
		<p class="mb-4">
			Which features are you most excited about? Select all that interest you.
		</p>
			...
			{#each FEATURES as feature}
				...
				<button
					...
					class="w-full text-left px-2 py-1.5 rounded transition-all cursor-pointer flex items-center gap-2.5 {isSelected
						...
						: 'hover:bg-surface-elevated'}"
				>
					<div class="w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 {isSelected
						? 'border-primary bg-primary'
						: 'border-border'}">
						{#if isSelected}
