<script lang="ts">
	...
	import type { Virtue, CreateSimpleRequest } from '$lib/types/axiology';

	let items: Virtue[] = $state([]);
	...
	async function loadItems() {
		loading = true;
		try {
			const response = await fetch(`${API_BASE}/axiology/virtues`);
			if (response.ok) {
				items = await response.json();
			}
		} catch (error) {
			console.error('Failed to load virtues:', error);
		} finally {
			loading = false;
		}
	}
	...
	async function createItem() {
		...
		try {
			...
			console.error('Failed to create virtue:', error);
		} finally {
			creating = false;
		}
	}
...
<Page>
	<div class="max-w-4xl">
		<div class="mb-8">
			<h1 class="text-3xl font-serif font-medium text-neutral-900 mb-2">Virtues</h1>
			<p class="text-neutral-600">Positive character patterns to cultivate</p>
		</div>
		...
		{:else}
			<div class="mb-6 border border-neutral-200 rounded-lg p-6">
				<h2 class="text-lg font-serif font-medium text-neutral-900 mb-4">Create New Virtue</h2>
				...
				<div class="space-y-4">
					<div>
						...
						<input
							...
							placeholder="e.g., Courage, Wisdom, Patience"
					...
					<div>
						...
						<textarea
							...
							placeholder="Describe this virtue and why you want to cultivate it..."
							rows="3"
							class="w-full px-3 py-2 border border-neutral-300 rounded-md"
						></textarea>
					</div>
		...
		{:else if items.length === 0}
			<div class="border border-neutral-200 rounded-lg p-12 text-center">
				<p class="text-neutral-500">No virtues yet</p>
			</div>
		{:else}
			<div class="space-y-3">
				{#each items as item}
