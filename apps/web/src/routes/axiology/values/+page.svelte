<script lang="ts">
	...
	import type { Value, CreateSimpleRequest } from '$lib/types/axiology';
	...
	async function loadItems() {
		loading = true;
		try {
			const response = await fetch(`${API_BASE}/axiology/values`);
			if (response.ok) {
...
<Page>
	<div class="max-w-4xl">
		<div class="mb-8">
			<h1 class="text-3xl font-serif font-medium text-neutral-900 mb-2">Values</h1>
			<p class="text-neutral-600">Foundational principles that guide your life</p>
		</div>
		...
		{:else}
			<div class="mb-6 border border-neutral-200 rounded-lg p-6">
				<h2 class="text-lg font-serif font-medium text-neutral-900 mb-4">Create New Value</h2>
				...
				<div class="space-y-4">
					<div>
						...
						<input
							...
							placeholder="e.g., Family, Growth, Authenticity"
					...
					<div>
						...
						<textarea
							...
							placeholder="What does this value mean to you? Why is it important?"
							rows="3"
							class="w-full px-3 py-2 border border-neutral-300 rounded-md"
						></textarea>
					</div>
		...
		{:else if items.length === 0}
			<div class="border border-neutral-200 rounded-lg p-12 text-center">
				<p class="text-neutral-500">No values yet</p>
			</div>
		{:else}
			<div class="space-y-3">
				{#each items as item}
