...
unsafe extern "C" {
    ...
    // nw_path_is_ultra_constrained is NOT declared here — see
    // `nw_path_is_ultra_constrained_checked` below. A plain extern declaration
    // makes it a hard link-time dependency, which is fatal below iOS 26.
...
/// VIRTUES PATCH — `nw_path_is_ultra_constrained` is `API_AVAILABLE(ios(26.0))`.
///
/// Upstream declares it in the `extern "C"` block and calls it unconditionally,
/// which makes the whole app unlaunchable on every iOS below 26:
///
///   • strong-linked → dyld aborts at launch, "Symbol not found:
///     _nw_path_is_ultra_constrained", before `main()` runs
///   • weak-linked   → the symbol binds to null and the process segfaults with
///     `pc = 0x0` the first time interfaces are enumerated
///
/// Both were observed on a TestFlight build running iOS 17.0. Rust cannot
/// express Apple's availability annotations, so the only way to keep the call
/// is to resolve it at runtime and fall back when the OS is too old. The field
/// is currently read by nothing — not netwatch, not iroh, not netdev itself —
/// so the fallback costs nothing in practice.
unsafe fn nw_path_is_ultra_constrained_checked(path: nw_path_t) -> bool {
    use std::sync::OnceLock;

    // dlsym(RTLD_DEFAULT, ...) searches every loaded image, which is where
    // Network.framework's symbols live once the framework is loaded.
    const RTLD_DEFAULT: *mut c_void = -2isize as *mut c_void;
    static SYM: OnceLock<Option<usize>> = OnceLock::new();

    let resolved = *SYM.get_or_init(|| {
        let name = b"nw_path_is_ultra_constrained\0";
        let ptr = libc::dlsym(RTLD_DEFAULT, name.as_ptr() as *const c_char);
        if ptr.is_null() {
            None
        } else {
            Some(ptr as usize)
    ...
    match resolved {
        Some(addr) => {
            let f: unsafe extern "C" fn(nw_path_t) -> bool = std::mem::transmute(addr);
            f(path)
        }
        // Pre-iOS 26: the concept does not exist, so nothing is ultra-constrained.
        None => false,
    }
}
...
fn collect_path_snapshot(path: nw_path_t) -> NWPathSnapshot {
    let mut snapshot = NWPathSnapshot {
        ...
        is_ultra_constrained: unsafe { nw_path_is_ultra_constrained_checked(path) },
