...
router.get('/auth', (req, res) => {
    try {
        ...
        const config = oauth_apps_1.oauthConfigs.google;
...
router.post('/refresh', async (req, res) => {
    try {
        ...
        if (!refresh_token) {
            throw (0, error_handler_1.createError)('Missing required parameter: refresh_token', 400);
        }
        ...
        if (!response.ok) {
            ...
            if (errorData.includes('invalid_grant')) {
                throw (0, error_handler_1.createError)('Refresh token is invalid or expired', 401);
            }
            throw (0, error_handler_1.createError)(`Token refresh failed: ${response.status}`, response.status);
        }
        const tokens = await response.json();
        if (!tokens.access_token) {
            throw (0, error_handler_1.createError)('No access token received from refresh', 500);
        }
