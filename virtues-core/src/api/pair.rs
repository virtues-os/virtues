...
// ─── Raw peer (wg-quick export) ────────────────────────────────────────────
//
// `virtues pair raw` runs ON THE BOX (direct DB access, no HTTP, no token —
// typing it IS proof of physical access, like `virtues pair`). It provisions a
// peer entirely box-side and prints a ready `wg-quick` config for a plain,
// non-Virtues client: a laptop that wants ssh over the tunnel, a router, an
// esp32. Reuses `assemble_bundle_generated` (box mints the keypair, returns the
// private half once) — the only divergences from `provision` are no bearer
// dance, no action fan-out (a raw peer is a network peer, not a collector), and
// a `.conf` output instead of a QR. It still gets a durable, revocable
// credential row, so it shows up in Devices and `DELETE /api/credentials/:id`
// evicts it from `wg0`.

/// Result of [`provision_raw_peer`]: the durable ids (for revocation) plus the
/// complete bundle (carrying the box-generated device private key) to render.
pub struct RawPeerProvision {
    pub device_id: String,
    pub credential_id: String,
    pub bundle: crate::wireguard::bundle::PairingBundle,
}

/// Provision a `raw_wg` peer and return everything needed to render its
/// `wg-quick` config. `label` is optional (defaults to a generic name shown in
/// the Devices list). Errors with a clear message on a non-Linux host, where
/// the WG engine — and thus a real config — isn't available.
pub async fn provision_raw_peer(
    pool: &PgPool,
    label: Option<String>,
) -> Result<RawPeerProvision, crate::Error> {
    const KIND: &str = "raw_wg";

    let label = label
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "raw peer (wg-quick)".to_string());
    let device_info = json!({ "client": "wg-quick", "added_via": "virtues pair raw" });

    let device_id =
        crate::ids::generate_id(crate::ids::DEVICE_PREFIX, &[KIND, &Utc::now().to_rfc3339()]);

    // Box generates the keypair (relay path), so no wg pubkey in the bearer pack.
    let bp = build_bearer_pack(KIND, &label, &Some(device_info.clone()), None)
        .map_err(|e| crate::Error::Other(format!("build bearer pack: {e:?}")))?;

    let mut tx = pool
        .begin()
        .await
        .map_err(|e| crate::Error::Other(format!("tx begin: {e}")))?;
    insert_device_row(&mut tx, &device_id, KIND, &label, &device_info, None)
        .await
        .map_err(|e| crate::Error::Other(format!("device insert: {e}")))?;
    // Permanent credential (expires_at = None) — minted on the box, not awaiting
    // a remote claim, so no provision-style claim deadline. A raw peer is not a
    // collector → source_id "__device__" (no action fan-out).
    insert_credential_row(&mut tx, &bp, "__device__", &label, &device_id, None)
        .await
        .map_err(|e| crate::Error::Other(format!("credential insert: {e}")))?;
    tx.commit()
        .await
        .map_err(|e| crate::Error::Other(format!("tx commit: {e}")))?;

    let _ = log_event(
        pool,
        Some(&device_id),
        "raw_wg_peer_provisioned",
        json!({ "label": &label, "credential_id": &bp.credential_id }),
        None,
        None,
    )
    .await;

    // Box-generated keypair → bundle carries `wg.client_private_key`. Also nudges
    // the daemon to reconcile wg0 now so the peer is live immediately.
    let bundle = assemble_wg_bundle_generated(pool, &bp.credential_id, &bp.bearer)
        .await?
        .ok_or_else(|| {
            crate::Error::Other(
                "WireGuard is only available on the Linux appliance — `virtues pair raw` \
                 can't generate a config on this host"
                    .into(),
            )
        })?;

    Ok(RawPeerProvision { device_id, credential_id: bp.credential_id, bundle })
}

/// Render a box-generated bundle as a `wg-quick` config for a raw peer. Pure —
/// no DB, no privileged op. The `[Interface]` private key is the box-generated
/// half from the bundle (standard `wg-quick` practice: the config you hand a
/// client already contains its private key); treat the output as a secret.
pub fn render_wg_quick_conf(bundle: &crate::wireguard::bundle::PairingBundle) -> String {
    use std::fmt::Write as _;
    let wg = &bundle.wg;
    let private = wg.client_private_key.as_deref().unwrap_or("<MISSING-PRIVATE-KEY>");
    // Route the whole ULA pool so the box (`::1`, == virtues.internal) and any
    // peer resolve through the tunnel; split-scope leaves the rest of the host's
    // traffic untouched.
    let pool = crate::wireguard::ula::pool_cidr();

    let mut s = String::new();
    let _ = writeln!(s, "[Interface]");
    let _ = writeln!(s, "PrivateKey = {private}");
    let _ = writeln!(s, "Address    = {}/128", wg.client_address);
    let _ = writeln!(s);
    let _ = writeln!(s, "[Peer]");
    let _ = writeln!(s, "PublicKey    = {}", wg.server_public_key);
    let _ = writeln!(s, "PresharedKey = {}", wg.preshared_key);
    let _ = writeln!(s, "Endpoint     = {}", wg.server_endpoint);
    let _ = writeln!(s, "AllowedIPs   = {pool}");
    // Keep the return path open through the box's firewall/NAT when the peer is
    // mostly idle (it dials out; the box can't initiate).
    let _ = writeln!(s, "PersistentKeepalive = 25");

    // Offer the box's other reachable addresses (LAN, etc.) as comments — swap
    // one into `Endpoint =` if the primary (global IPv6) isn't reachable from
    // wherever this peer runs.
    let alts: Vec<&String> =
        wg.server_endpoints.iter().filter(|e| **e != wg.server_endpoint).collect();
    if !alts.is_empty() {
        let _ = writeln!(s);
        let _ = writeln!(s, "# Alternate box endpoints (swap into Endpoint = … as needed):");
        for e in alts {
            let _ = writeln!(s, "#   {e}");
        }
    }
...
#[cfg(test)]
mod raw_conf_tests {
    use super::render_wg_quick_conf;
    use crate::wireguard::bundle::{PairingBundle, WgParams};

    fn bundle() -> PairingBundle {
        PairingBundle {
            bearer: "unused-by-conf".into(),
            wg: WgParams {
                server_public_key: "Y1twqKw8umjdxT788eiUiz9Owqo9kR+IxFWfyIOsLQ4=".into(),
                server_endpoint: "[2603:8080:1500:1d00::1]:51820".into(),
                server_endpoints: vec![
                    "[2603:8080:1500:1d00::1]:51820".into(),
                    "192.168.1.233:51820".into(),
                ],
                preshared_key: "cHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHM=".into(),
                client_address: "fd00:5654::9".into(),
                server_address: "fd00:5654::1".into(),
                allowed_ips: vec!["fd00:5654::1/128".into()],
                client_private_key: Some("cHJpdmF0ZWtleXByaXZhdGVrZXlwcml2YXRla2V5MDA=".into()),
            },
    ...
    #[test]
    fn renders_a_valid_wg_quick_conf() {
        let c = render_wg_quick_conf(&bundle());
        // Interface uses the box-generated private key + the assigned /128.
        assert!(c.contains("PrivateKey = cHJpdmF0ZWtleXByaXZhdGVrZXlwcml2YXRla2V5MDA="));
        assert!(c.contains("Address    = fd00:5654::9/128"));
        // Peer = the box, with PSK, primary endpoint, pool route, keepalive.
        assert!(c.contains("PublicKey    = Y1twqKw8umjdxT788eiUiz9Owqo9kR+IxFWfyIOsLQ4="));
        assert!(c.contains("PresharedKey = cHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHNrcHM="));
        assert!(c.contains("Endpoint     = [2603:8080:1500:1d00::1]:51820"));
        assert!(c.contains("AllowedIPs   = fd00:5654::/64"));
        assert!(c.contains("PersistentKeepalive = 25"));
        // The LAN address is offered as a swappable comment, not the live Endpoint.
        assert!(c.contains("#   192.168.1.233:51820"));
    }

    #[test]
    fn missing_private_key_is_flagged_not_silently_blank() {
        let mut b = bundle();
        b.wg.client_private_key = None;
        let c = render_wg_quick_conf(&b);
        assert!(c.contains("PrivateKey = <MISSING-PRIVATE-KEY>"));
    }
}
