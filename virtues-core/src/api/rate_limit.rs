...
pub async fn check_rate_limit(
    ...
    // Postgres uses 'excluded' for conflict values, and we need to reference the table directly
    ...
    let new_count: i64 = sqlx::query_scalar!(
        ...
        DO UPDATE SET
            request_count = app_api_usage.request_count + 1,
            updated_at = now()
    ...
    if new_count > daily_request_limit as i64 {
        ...
        return Err(RateLimitError::DailyLimitExceeded {
            current: new_count.min(i32::MAX as i64) as i32,
...
pub async fn record_usage(
    ...
    // BIGINT columns — i64 is wide enough that overflow protection isn't needed
    // for any practical request count or token sum.
    let total_tokens: i64 = tokens.total() as i64;
    let input_tokens: i64 = tokens.input as i64;
    let output_tokens: i64 = tokens.output as i64;
...
async fn get_daily_usage(
    ...
    // Explicit type annotations needed for COALESCE results
    let result = sqlx::query!(
        r#"
        SELECT
...
async fn get_daily_cost(pool: &PgPool, day_bucket: NaiveDate) -> Result<f64, sqlx::Error> {
    let result = sqlx::query!(
        r#"
...
/// (`virtues_registry::models::get_model_pricing`, rates per 1K tokens).
fn calculate_cost(tokens: &TokenUsage) -> f64 {
    let (input_per_1k, output_per_1k) =
        virtues_registry::models::get_model_pricing(&tokens.model);
    let input_cost = (tokens.input as f64 / 1000.0) * input_per_1k;
    let output_cost = (tokens.output as f64 / 1000.0) * output_per_1k;
