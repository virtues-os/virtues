...
/// Perform a web search using Exa AI (proxied through virtues-api)
pub async fn search(pool: &PgPool, request: SearchRequest) -> Result<SearchResponse> {
    ...
    let body = serde_json::to_value(&exa_request)
        .map_err(|e| Error::ExternalApi(format!("Failed to serialize Exa request: {}", e)))?;

    let response = BearerClient::from_env(pool.clone())
        .post_json("/v1/exa/search", &body)
        .await
        .map_err(|e| Error::ExternalApi(format!("virtues-api/Exa request failed: {}", e)))?;

    if !response.is_success() {
        return Err(Error::ExternalApi(format!(
            "virtues-api/Exa error ({}): {}",
            response.status, response.body
        )));
    }

    let exa_response: ExaApiResponse = serde_json::from_value(response.body)
