...
async fn get_enabled_tools_map(db: &PgPool) -> Result<HashMap<String, bool>> {
    let row = sqlx::query!(
        r#"
    ...
    let enabled_tools: HashMap<String, bool> = serde_json::from_value(row.enabled_tools)
...
pub async fn list_tools(db: &PgPool, params: ListToolsQuery) -> Result<Vec<Tool>> {
    ...
    // 2. Get MCP tools from the database
