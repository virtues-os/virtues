...
async fn get_profile(db: &PgPool) -> Result<AssistantProfile> {
...
async fn save_personas_data(db: &PgPool, data: &PersonasData) -> Result<()> {
    let json = serde_json::to_value(data)
        .map_err(|e| Error::Other(format!("Failed to serialize personas: {}", e)))?;

    sqlx::query("UPDATE app_assistant_profile SET personas = $1, updated_at = now()")
        .bind(&json)
        .execute(db)
        .await
...
async fn seed_if_empty(db: &PgPool, data: &mut PersonasData) -> Result<bool> {
...
pub async fn list_personas(db: &PgPool) -> Result<Vec<Persona>> {
...
pub async fn list_all_personas(db: &PgPool) -> Result<(Vec<Persona>, Vec<String>)> {
    let profile = get_profile(db).await?;
    let mut data = parse_personas_data(&profile);
...
pub async fn get_persona(db: &PgPool, id: &str) -> Result<Option<Persona>> {
    let profile = get_profile(db).await?;
...
pub async fn get_persona_content(db: &PgPool, id: &str) -> Result<Option<String>> {
    let persona = get_persona(db, id).await?;
    Ok(persona.map(|p| p.content))
}

/// Create a new custom persona
pub async fn create_persona(db: &PgPool, req: CreatePersonaRequest) -> Result<Persona> {
    let profile = get_profile(db).await?;
    let mut data = parse_personas_data(&profile);
...
pub async fn update_persona(db: &PgPool, id: &str, req: UpdatePersonaRequest) -> Result<Persona> {
    let profile = get_profile(db).await?;
    let mut data = parse_personas_data(&profile);
...
pub async fn hide_persona(db: &PgPool, id: &str) -> Result<()> {
    let profile = get_profile(db).await?;
    let mut data = parse_personas_data(&profile);
...
pub async fn unhide_persona(db: &PgPool, id: &str) -> Result<()> {
    let profile = get_profile(db).await?;
    let mut data = parse_personas_data(&profile);
...
pub async fn reset_personas(db: &PgPool) -> Result<Vec<Persona>> {
    // Clear existing data
