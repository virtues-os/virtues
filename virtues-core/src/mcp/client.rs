...
//! Provides lazy reconnection — tool metadata is cached in Postgres, connections
...
impl McpClientManager {
    ...
    pub async fn connect(&self, server_id: &str) -> Result<usize, ToolError> {
        ...
        let _ = sqlx::query(
            "UPDATE app_mcp_servers SET status = 'connected', last_connected_at = now(), last_error = NULL WHERE id = $1",
        )
        .bind(server_id)
        .execute(self.pool.as_ref())
        .await;
