...
/// Convert Postgres rows to JSON array with comprehensive type support
///
        ...
        for (i, col) in row.columns().iter().enumerate() {
            ...
            let value: serde_json::Value =
                ...
                // Fallback: try to get raw value info for better error message
                    ...
                    tracing::warn!(
                        "Column '{}' with Postgres type '{}' could not be extracted - falling back to placeholder",
                        col_name,
                        type_info
                    );
