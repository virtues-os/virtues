...
//!
//! Runs after `sqlx::migrate!` in `database::initialize()`. migrations
...
async fn log_legacy_table_baseline(db: &PgPool) {
    let exists: Option<(String,)> = sqlx::query_as(
        "SELECT tablename FROM pg_tables WHERE schemaname = 'public' \
         AND tablename = 'action_credentials'",
    )
    .fetch_optional(db)
    .await
    .ok()
...
async fn reencrypt_pending_credentials(db: &PgPool) -> Result<()> {
    let pending: Vec<(String, String)> = sqlx::query_as(
        r#"SELECT id, status FROM credentials
           WHERE secrets_ciphertext = $1"#,
    )
    .bind(PLACEHOLDER)
    .fetch_all(db)
    .await
...
async fn reencrypt_one(
    ...
    let legacy: Option<(
        ...
        r#"SELECT device_id, device_token, device_info::text, metadata::text
           FROM action_credentials WHERE id = $1"#,
    )
    .bind(cred_id)
    .fetch_optional(db)
    .await
