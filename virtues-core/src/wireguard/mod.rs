...
//! `docs/deployment.md`. The wire-protocol types (pair bundle, constants, SPKI
//! helpers) live in `virtues-protocol` so cross-platform daemons (iOS, Mac,
//! Android, ESP32) can decode them without dragging the box binary's dep tree.
//!
...
pub use virtues_protocol::{
    bundle, constants, spki, INTERNAL_HOST, INTERNAL_PORT, LAN_HOST,
};
