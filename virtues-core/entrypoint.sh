...
if [ "${SEED_DEMO:-}" = "true" ]; then
    ...
    exec su -s /bin/sh virtues -c "virtues bringup && virtues seed && virtues server"
else
    # `bringup` = migrate + ensure box identity (CA, rendezvous, WG keypair),
    # idempotent. Same first-boot path on the appliance and a DIY install.
    exec su -s /bin/sh virtues -c "virtues bringup && virtues server"
fi
