-- RENUMBERED from 0099. HEAD carries 0100-0103 (an earlier 0099 was committed
-- and reverted as "half of somebody else's change"), so slotting a new migration
-- BENEATH four that already shipped is the out-of-order case that breaks a box
-- mid-upgrade. Every statement below is therefore guarded and re-runnable: a
-- checkout that already applied the old 0099 by hand converges here silently
-- instead of aborting on "relation wiki_entity_refs does not exist".
--
-- ---------------------------------------------------------------------------
...
DO $$
BEGIN
    IF to_regclass('public.wiki_entity_refs') IS NOT NULL
       AND to_regclass('public.wiki_refs') IS NULL THEN
...
ALTER TABLE wiki_refs DROP CONSTRAINT IF EXISTS wiki_refs_subject_type_check;
...
DO $$ BEGIN
    IF to_regclass('public.idx_entity_refs_entity') IS NOT NULL AND to_regclass('public.idx_wiki_refs_subject') IS NULL THEN
        ALTER INDEX idx_entity_refs_entity RENAME TO idx_wiki_refs_subject;
    END IF;
END $$;
DO $$ BEGIN
    IF to_regclass('public.idx_entity_refs_source') IS NOT NULL AND to_regclass('public.idx_wiki_refs_source') IS NULL THEN
        ALTER INDEX idx_entity_refs_source RENAME TO idx_wiki_refs_source;
    END IF;
END $$;
DO $$ BEGIN
    IF to_regclass('public.idx_entity_refs_type') IS NOT NULL AND to_regclass('public.idx_wiki_refs_subject_type') IS NULL THEN
        ALTER INDEX idx_entity_refs_type RENAME TO idx_wiki_refs_subject_type;
    END IF;
END $$;
DO $$ BEGIN
    IF to_regclass('public.idx_entity_refs_source_type') IS NOT NULL AND to_regclass('public.idx_wiki_refs_source_subject_type') IS NULL THEN
        ALTER INDEX idx_entity_refs_source_type RENAME TO idx_wiki_refs_source_subject_type;
    END IF;
END $$;
DO $$ BEGIN
    IF to_regclass('public.idx_entity_refs_unique') IS NOT NULL AND to_regclass('public.idx_wiki_refs_unique') IS NULL THEN
...
COMMENT ON COLUMN wiki_refs.entity_type IS
    ...
    'column name (see the header of this migration on why the rename was not worth its blast '
