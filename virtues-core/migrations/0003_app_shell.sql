...
-- ---------------------------------------------------------------------------
-- Spaces (the "room" a chat lives in)
--
-- A Space is a manual collection — a project, pet, hobby, goal, or topic you
-- return to. It gathers entities, chats, and pages (URL-native membership) and
-- a chat may live in at most one Space (see app_chats.space_id in 0005). Being
-- "in" a Space weights its members in retrieval and, conversely, folds the
-- chat into the Space's corpus.
--
-- `accent_color` is the single user-chosen tint for the room (ambient surface
-- accent, not a per-badge color). `current_status` is the catch-up memo shown
...
CREATE INDEX idx_app_spaces_name ON app_spaces(name);
CREATE INDEX idx_app_spaces_sort ON app_spaces(sort_order, created_at);
CREATE TRIGGER set_updated_at BEFORE UPDATE ON app_spaces
    FOR EACH ROW EXECUTE FUNCTION tg_set_updated_at();

-- Space membership — URL-native (/person/abc, /chat/xyz, /page/…, external https://).
