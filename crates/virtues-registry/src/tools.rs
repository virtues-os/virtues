...
fn record_introductions_tool() -> ToolConfig {
    ToolConfig {
        ...
        llm_description: "Write down what the person said about themselves: their full name, what to call them, what they will call you, the city they live in with the IANA time zone you resolve from it, and their birth date (YYYY-MM-DD). Include only what they gave; leave the rest out rather than guessing, and each field you omit is left as it was. Call this the moment you have anything — it writes immediately and shows what it wrote under your turn, so do not ask them to confirm and do not list the fields back in your own words. If they gave only a first name, or left out the birth date, or gave a year without a day, ask once for what is missing. If they correct something afterwards, call this again with only what changed.".to_string(),
        parameters: serde_json::json!({
            "type": "object",
            "properties": {
