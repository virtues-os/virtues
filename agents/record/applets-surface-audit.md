...
| ~~**L5**~~ | `AppletsPanel.svelte:355` | **FIXED.** The empty state now shows what to ask for. (The plan's curated "Practices" gallery is cut — chat is the front door, and a second catalog surface earns nothing.) | `+ New` points at chat |
...
| ~~**M2**~~ | 16 manifests | **FIXED.** `schedule` is canonical everywhere; `default_cron` survives only as a serde alias so an older folder still loads | manifest key → `schedule` in phase 1 |
| ~~**M3**~~ | `MANIFEST_SCHEMA.json`, `AUTHORING.md` | **FIXED.** Zero occurrences of "action" left in either. The schema drops `runtime` and its `oneOf`, gains `ai` as an owner, and its examples are real applets; AUTHORING.md is rebuilt around field-presence and points declarative authors at AGENTS.md | phase 6 cleanup: AUTHORING.md → AGENTS.md + doc-drift fix |
...
| ~~**A1**~~ | `applets/user/heart_rate_explorer/manifest.toml` | **FIXED.** A face-only dashboard carried a no-op prompt (*"If run, do nothing and report that this is a display-only dashboard"*), so "Run now" spent a model call to say nothing. Removed from the folder on this box — untracked state-root data, so not in the commit — and the cause is closed by A2 | capability contract must be exhaustive; the model writes only what it is told exists |
| ~~**A2**~~ | `applets/AGENTS.md` | **FIXED.** `agent` is now documented as optional, `description` says outright that the user reads it and what voice to use, and a section states plainly that a dashboard has no agent | the sentence is compiled from intent and blessed at the gate |
