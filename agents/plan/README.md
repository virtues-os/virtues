...
| [audio-schedule-places-plan.md](audio-schedule-places-plan.md) | Planned | When and where the mic listens: quiet hours generalize to a per-weekday schedule, and a short list of places on the phone mutes inside them. Both are gates on the tap (mute, never release); a muted marker lets the box tell "kept nothing, by choice" from a dead collector. |
...
| [getting-started-plan.md](getting-started-plan.md) | Planned | After the founder's letter, getting started is ONE chat room: connect AI, introductions, sources, the interview hand-off. Steps are derived from the record, never stored; the model is a guest who arrives after step 1; the app is locked until the box can call a model, with `/dangerously-skip-onboarding` as the ugly exit. Supersedes the seven-step Home page. |
...
| [observability-plan.md](observability-plan.md) | Planned | Keys on every journal line (`run_id`, `request_id`, …) so a failed run can find its own log, one `POST /api/events` door so a client failure becomes a box fact, and the crash beacon made honest — it posts to the cloud today without ever recording the crash locally. Events table and Logbook UI deliberately deferred. |
