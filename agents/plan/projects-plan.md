# Projects — paradigm + implementation plan

*Renamed 2026-09-21: Notebook became **Project** (`app_notebooks` → `app_projects`, `app_notebook_items` → `app_project_items`, `app_chats.notebook_id` → `project_id`, `/notebook/…` → `/project/…`, `/api/notebooks` → `/api/projects`; ids keep the `nb_` prefix). Sections marked as done keep the names they shipped with.*

Status: planning (2026-07-08). Renames + folds the half-built "Spaces" paradigm into a
first-class **Project** — a workspace lens over your life-graph, with a **Library** of
materials (files + external snapshots + pasted text), native text extraction, project-scoped
...
- **Project** = a workspace you *enter and work in*. A saved, named lens over the graph:
  ...
  Project = topic-slice; pages/chats/entities/assets are the primitives both slice.)
...
- Internal metaphor = **room** (you enter it; it has a state and an accent tint). User-facing
  name = Project. Don't name the feature "Rooms".

### Naming (resolved; REVISED 2026-07-20 — see researcher-plan.md decision 0)

- ~~The materials in a project = its **Library**~~ **SUPERSEDED**: the "Library" noun is
  retired — a project *containing* a Library was a container-inside-container that
  contradicted the lens model. Things are simply **in the project**; the verb is
  **"Add to project"**, the contents are **project items** (`app_project_items`).
  ...
  item); `role='pin'` is schema-only for nav-only edges (e.g. related projects).
- **Items = anything retrievable, not just files.** Files, external snapshots, pasted text,
  ...
  project's retrieval scope regardless of type. This is the federation superpower (don't ape
  NotebookLM's upload-bin).
- A project has **no local notes** — just global **Pages** it references. "Paste raw text"
...
|-----------|-----|---------|
| Spaces | collection + chat-binding + memo + color | **Projects** |
| Things (as container) | "folder you re-enter: projects/pets/goals/topics" | **removed** — fold into Projects; Thing stays as pure ER entity |
| Pins | flat global URL shortcuts | **keep** (different axis: cross-cutting quick-nav) |
| Tags (on pages) | stored, no UI | **keep as input to projects** (future smart membership); no browsing UI |

Clean three tiers: **Projects** (curated workspaces) · **Pins** (flat shortcuts) ·
**Tags** (labels that feed projects).

## Doctrines (decided)

- **Chat = many threads, Claude-Projects style.** A project shares context (Library +
  ...
  (This is also less work: the data already binds many chats to one project.)
...
- **One asset viewer for all MIME types**, reused full-page, in a lightbox, and in the project
  split-pane.
...
Rename + consolidate:
- `app_spaces` → `app_projects`; add `instructions TEXT` (persistent system prompt, distinct
  from the transient `current_status` memo).
- `app_space_items` → `app_project_items`; add `role TEXT NOT NULL DEFAULT 'pin'` (`library` | `pin`).
- `app_chats.space_id` → `project_id`.
...
- Project-scoped query: resolve `role='library'` members → asset ids + entity ids, then
...
AssetHeader.svelte           title, MIME badge, size, download, Add-to-Project, in-N-projects
Hosts:
  ...
  AssetPane.svelte               project split view (Library │ viewer │ chat)
```

Project home = tabbed: **Overview** (state-of-room + instructions) · **Library** · **Chats** · **Map**.
- Project home is a **built-in default view** (plain Svelte components — NOT the `view`-applet
...
- **Map** = an auto **concept graph**, not files-linking-files. Nodes = entities referenced/extracted
  across the project's materials + pages; edges = co-occurrence; clusters = themes. Actionable:
...
Routes: canonical `/asset/{id}`; alias `/drive/file_{id}`. Projects at `/projects` + `/project/{id}`.
...
```
Project ──(item role=library)──▶ /asset/file_x
Asset ──"in N projects" + Add-to-Project──▶ Project
Asset ──[@ref]──▶ page/chat (ref-picker already finds files)
Asset ──extracted+embedded──▶ project-scoped search ──▶ chat answer
Chat citation ──deep-link──▶ /asset/file_x#page=3
Annotation ──promote──▶ note-page ──pin──▶ Project   (later)
```

## Order: first → last

**First (foundation + de-risk):**
1. Rename Spaces→Notebooks (since renamed to Projects), **remove Things-as-container**, and apply the **sidebar restructure**
...
4. Extraction → embed (`ontology='asset'`) + **project-scoped retrieval** + **source-level
   citations** (page-level for PDFs via `#page=N`). Many grounded chats per project.
...
*Done under the Notebook name; the identifiers below are the ones that shipped on 2026-07-09 and were renamed again on 2026-09-21.*
...
**(1) Surface the new columns** — `api/projects.rs`:
- `Project` + `ProjectSummary` structs: add `instructions: Option<String>`, `archived_at: Option<Timestamp>`.
- `ProjectItem` struct: add `role: String`.
- Add `instructions, archived_at` to every project SELECT/RETURNING: `list_projects` (~L110),
  `get_project` (~L131), `create_project` RETURNING (~L173), `update_project` SELECT+RETURNING (~L193,234).
- Add `role` to the item SELECTs: `get_project` items (~L145) + `add_project_item` RETURNING (~L304).
- `UpdateProjectRequest`: add `instructions: Option<Option<String>>` (tri-state) + `archived: Option<bool>`
  (→ set/clear `archived_at`); extend the UPDATE SET + binds.
- `client.ts`: add `instructions`/`archived_at` to `Project` (Summary+Detail inherit) and `role` to
  `ProjectItem`; add `instructions?`/`archived?` to the store `update()` patch type.
- `role` is READ-ONLY in 1.5 (setting library-vs-pin is Phase 3); `add_project_item` keeps DB default 'pin'.

**(2) Split memo vs instructions + wire into chat** — `chat.rs build_project_context` (~L767):
- Add an `<instructions>` element (from `detail.project.instructions`) distinct from `<memo>`
  (`current_status`); update the preamble to describe both (instructions = persistent behavior,
  memo = transient catch-up). get_project already returns it once the struct carries it.
- Optional (deferrable): a small "Instructions" textarea in `ProjectDetailView` beside the memo.
...
- Add `project_id: Option<&str>` + `ScopeMode { Boost, Strict }` to `search()` (query.rs).
...
- **Free win:** `ToolContext.project_id` is ALREADY populated from chat.rs but the
  `semantic_search` tool ignores it. Thread it through (tool → `engine.search(project_id, Boost)`).
- **Decisions:** dynamic member resolution per search call (not snapshot — adding a material
  mid-chat should count); reject `/project/` as a `library` member (cycle risk; allow as `pin`).
...
- **Decision:** Map v1 = graph of entities **explicitly referenced** (`[@ref]` links + structured
  `wiki_entity_refs`) across the project's materials — buildable on existing data. Auto-NER-over-
...
- Add a `chat_mode`/scope value (`grounded` vs `open`) to `ChatRequest` (sits beside `agent_mode`);
  `grounded` → `ScopeMode::Strict` + a prompt line "answer only from this project's materials, cite
  everything, else say it's not in the materials." The PhD-trust toggle.

### Pins / tags — no change, no dead code
- Keep the three tiers distinct (Projects / Pins / Tags). Tag-browsing stays deferred; tags remain
...
- **Templates + suggest-a-project** (adoption on-ramp) — much later, not worth it now.
- Extra views (**Timeline**, **Outline/Board**) — built-in modules could be added later; not in v1.
- **Project views as customizable `view` applets** — explicitly out. Built-in default only.

---

## Phase C — Citations (source-anchored, Ref-native)  ← NEXT

**Goal:** when the model answers, a load-bearing claim carries a **named source chip** (the source's own name + icon) that **opens the real source** — the page, person, day, project source, or uploaded doc it rests on. Not a generic "5 matches" blob, not an opaque `[1]`.

**Decided (2026-07-10):** named source chips · click opens the real source (split-pane) · cite load-bearing claims only · applies everywhere retrieval runs (not just projects).
...
Ref routes exist for `page / day / person / place / org / thing / source / chat / project / file`. But a `semantic_search` hit is often a **raw ontology record** — an email, calendar event, transaction — which has **no viewer route**. "Open the real source" can't point at those. Resolution for v1:
...
- **In a project, fall back to the owning member** (which is always viewable — that's what a member *is*).
...
**Goal:** a project can hold an uploaded PDF / text / doc whose **native text** is extracted, embedded, retrievable (scoped, via Phase C's citations), and viewable. This is the researcher/PhD archetype. Bigger than C; sequence it after.
...
- **Project membership already stores `/drive/file_<id>` URLs**; `resolve_project_scope` just skips them today (`virtues-core/src/search/query.rs:171`).
...
4. **Wire `/drive/file_<id>` into project scope.** In `resolve_project_scope` (`query.rs:157-171`), resolve a `/drive/file_` member to its chunk record-ids so scoped retrieval + boost include the doc. Phase C citations then deep-link a hit to `AssetView` **at its `page_num`**.
...
### Sequencing within D
Extraction + chunk table + ontology (1–3) are the spine — ship that first and confirm an uploaded PDF becomes searchable. Then scope-wiring (4) lights up project grounding for docs. Range (5) and viewer polish (6) are quality passes, last.
...
### Done when
Drop a PDF into a project → it extracts + embeds → ask the project → answer cites the PDF → clicking the pill opens the doc at the cited page.
...
3. **Phase D wiring** — `/drive/file_` into project scope; C's citations now cover docs.
