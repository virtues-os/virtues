...
| **Inference** | Two endpoints you run: `/v1/embeddings` (required) and `/v1/rerank` (optional), on loopback, LAN, or VPN — never a public address. [Setting up inference](docs/inference.md) has the commands and the models. A bundled CPU-only quick trial exists for a first look. |
