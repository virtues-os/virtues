---
title: The CLI
description: The virtues command — checking the box's health, pairing devices, moving between releases, and the maintenance verbs you'll actually use.
updated: 2026-08-28
---

Installing puts a `virtues` binary on the box. It's the same program that
...
Box health in one screen: identity, subscription, and which devices are
...
Pairing a device is the one human verb for connecting something to your box:
...
wiping the box, revoking the last device other than itself, and importing an
...
are, and you can, by sitting down at the box.
...
virtues channel            # print the channel this box follows
...
a one-off override that forgets itself, so a box meant to track staging drifts
...
box, and `virtues activate` installs what it staged.

## Data

```bash
sudo -u virtues virtues backup     # snapshot the box into one tarball
sudo virtues restore <tarball>     # replace this box's state from one
...
makes the tarball **as sensitive as the box itself**. See
...
re-embed from source. Run it when the box reports a fingerprint or dimension
mismatch.
...
asks you to type the box's hostname to confirm. Shared infrastructure it
