...
### Steps
1. ✅ **Emit a `ref` per result (backend).** `semantic_search` results carry `ref: Option<String>` (`SearchResult.ref_url`, serialized `ref`). Resolution is **entity-anchored**: `resolve_refs` batch-looks-up each hit's highest-confidence `wiki_entity_refs` link and maps `entity_type` → route (`/person`,`/place`,`/org`,`/thing` via `entity_route_base`). No link → no `ref` → no chip. (Every searchable ontology is a raw `data_*` record with no viewer of its own, so the entity is the navigable source. Day-anchoring was rejected for v1: `day_<sha8>` ids aren't guaranteed materialized → dead-link risk.)
...
**Still to verify (runtime, not compile):** entity-ref coverage on real data (how often hits actually carry a `wiki_entity_refs` link → how visible citations are); and the notebook-scoped boost (separately untested). If coverage is thin, revisit day-anchoring (with an existence check) or notebook-member fallback.
