...
> deleted outright and answer UnknownCommand; 0x83 is codeless — the old
> five-string shape is a malformed packet); RPC 0x87 was CUT (pair-last with
> the box-side complete-ticket wait made a separate ticket read pointless);
> the codeless pair fetches the box's OWN standing code internally
> (ensure_standing) so the consume transaction and rate-limit story are
> unchanged. The airlock's LAN code form remains the only place a human can
