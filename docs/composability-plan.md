...
`vector(256)` with a hardcoded 768→256 Matryoshka truncation**. A manual
endpoint with e.g. 384-dim output will misbehave. Needed: create/alter the
vector column from probed dims at bringup (migration 0017 is the pattern),
and make the truncation target configurable (`VIRTUES_EMBED_DIMS` as the
stored width; keep 768→256 as the Dragon default).
