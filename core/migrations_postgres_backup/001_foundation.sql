-- Foundation: Schemas, extensions, utility functions, pipeline infrastructure
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_source_connections_source_device
    ON data.source_connections(source, device_id) WHERE device_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_source_connections_device_token
    ON data.source_connections(device_token) WHERE device_token IS NOT NULL;

DROP TRIGGER IF EXISTS data_source_connections_set_updated_at ON data.source_connections;
...
DROP TRIGGER IF EXISTS data_stream_connections_set_updated_at ON data.stream_connections;
CREATE TRIGGER data_stream_connections_set_updated_at
...
CREATE INDEX IF NOT EXISTS idx_jobs_pending
    ON data.jobs(created_at ASC) WHERE status = 'pending';
CREATE INDEX IF NOT EXISTS idx_jobs_parent
    ON data.jobs(parent_job_id) WHERE parent_job_id IS NOT NULL;

DROP TRIGGER IF EXISTS data_jobs_set_updated_at ON data.jobs;
...
CREATE INDEX IF NOT EXISTS idx_stream_objects_source_stream
    ON data.stream_objects(source_connection_id, stream_name);
CREATE INDEX IF NOT EXISTS idx_stream_objects_timestamp_range
    ON data.stream_objects(source_connection_id, stream_name, min_timestamp, max_timestamp);
CREATE INDEX IF NOT EXISTS idx_stream_objects_archive_job
    ON data.stream_objects(archive_job_id) WHERE archive_job_id IS NOT NULL;

DROP TRIGGER IF EXISTS data_stream_objects_set_updated_at ON data.stream_objects;
CREATE TRIGGER data_stream_objects_set_updated_at
    BEFORE UPDATE ON data.stream_objects
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX IF NOT EXISTS idx_transform_checkpoints_lookup
    ON data.stream_transform_checkpoints(source_connection_id, stream_name, transform_name);

DROP TRIGGER IF EXISTS data_stream_transform_checkpoints_set_updated_at ON data.stream_transform_checkpoints;
CREATE TRIGGER data_stream_transform_checkpoints_set_updated_at
    BEFORE UPDATE ON data.stream_transform_checkpoints
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX IF NOT EXISTS idx_stream_checkpoints_lookup
    ON data.stream_checkpoints(source_id, stream_name, checkpoint_key);

DROP TRIGGER IF EXISTS data_stream_checkpoints_set_updated_at ON data.stream_checkpoints;
CREATE TRIGGER data_stream_checkpoints_set_updated_at
    BEFORE UPDATE ON data.stream_checkpoints
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();
...
CREATE INDEX IF NOT EXISTS idx_archive_jobs_pending
    ON data.archive_jobs(status, created_at) WHERE status IN ('pending', 'failed');
CREATE INDEX IF NOT EXISTS idx_archive_jobs_source
    ON data.archive_jobs(source_connection_id, stream_name, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_archive_jobs_sync_job
    ON data.archive_jobs(sync_job_id) WHERE sync_job_id IS NOT NULL;

DROP TRIGGER IF EXISTS data_archive_jobs_set_updated_at ON data.archive_jobs;
CREATE TRIGGER data_archive_jobs_set_updated_at
    BEFORE UPDATE ON data.archive_jobs
    FOR EACH ROW EXECUTE FUNCTION update_updated_at();

-- Add FK after archive_jobs exists
ALTER TABLE data.stream_objects
    ADD CONSTRAINT fk_stream_objects_archive_job
    FOREIGN KEY (archive_job_id) REFERENCES data.archive_jobs(id) ON DELETE SET NULL;
