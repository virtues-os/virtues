...
//! - `function_name` set  → subprocess binary
...
pub async fn get_enabled_actions(db: &SqlitePool) -> Result<Vec<Action>> {
    let rows = sqlx::query("SELECT * FROM app_actions WHERE enabled = 1 ORDER BY name")
        .fetch_all(db)
        .await?;
...
pub async fn update_memory(db: &SqlitePool, action_id: &str, memory: &str) -> Result<()> {
    let affected = sqlx::query(
        "UPDATE app_actions SET memory = ?, updated_at = datetime('now') WHERE id = ?",
    )
    .bind(memory)
    .bind(action_id)
    .execute(db)
...
/// Detect a SQLite UNIQUE constraint violation from a `DatabaseError`.
fn is_unique_violation(dbe: &dyn sqlx::error::DatabaseError) -> bool {
    // SQLite error code for SQLITE_CONSTRAINT_UNIQUE is 2067; the extended
    // code is exposed via `code()` as a Cow<str>.
...
pub async fn update_action(
    ...
    // Build the UPDATE statement. SQLite has no "set only these keys" shortcut;
    ...
    sqlx::query(
        r#"UPDATE app_action_runs
           SET status = ?, completed_at = datetime('now'),
               records_processed = ?, error = ?, result_summary = ?
           WHERE id = ?"#,
    )
    .bind(status)
...
pub fn action_from_row(row: &sqlx::sqlite::SqliteRow) -> Result<Action> {
    let triggers_raw: String = row.try_get("triggers").unwrap_or_else(|_| "[]".to_string());
