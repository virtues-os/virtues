...
pub async fn resolve_sleep_events(pool: &SqlitePool, date: NaiveDate) {
...
/// If not, creates one.
async fn resolve_sleep_for_date(pool: &SqlitePool, date: NaiveDate) {
    use sqlx::Row;
    ...
    let existing: Option<String> = sqlx::query_scalar(
        "SELECT id FROM wiki_events WHERE day_id = $1 AND is_sleep = 1 LIMIT 1",
    )
    .bind(&day_id)
    .fetch_optional(pool)
    .await
