...
pub async fn build_hourly_context(
    ...
    if let Ok(row) = sqlx::query(
        "SELECT COUNT(*) as cnt, AVG(bpm) as avg_bpm FROM data_health_heart_rate WHERE timestamp >= ? AND timestamp < ?",
    )
    .bind(&start)
    .bind(&end)
    .fetch_optional(pool)
    ...
    if let Ok(row) = sqlx::query(
        "SELECT SUM(step_count) as total FROM data_health_steps WHERE timestamp >= ? AND timestamp < ?",
    )
    .bind(&start)
    .bind(&end)
    .fetch_optional(pool)
