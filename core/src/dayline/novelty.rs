...
pub async fn compute_novelty_for_day(pool: &SqlitePool, date: NaiveDate) -> anyhow::Result<u32> {
...
async fn load_baseline(pool: &SqlitePool, event_date: NaiveDate) -> anyhow::Result<Option<Baseline>> {
...
/// Serialize f32 embedding to little-endian bytes for SQLite BLOB storage.
fn embedding_to_bytes(embedding: &[f32]) -> Vec<u8> {
    embedding.iter().flat_map(|f| f.to_le_bytes()).collect()
}

/// Deserialize little-endian bytes from SQLite BLOB to f32 embedding.
