...
pub async fn compute_autonomic_for_day(pool: &SqlitePool, date: NaiveDate) -> anyhow::Result<u32> {
...
async fn get_resting_hr(pool: &SqlitePool, before_date: &str) -> Option<f64> {
...
async fn get_resting_hr_std(pool: &SqlitePool, before_date: &str) -> Option<f64> {
