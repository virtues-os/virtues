    ...
    async fn transform(
        ...
        for batch in batches {
            for record in &batch.records {
                ...
                let Some(audio_file_key) = record.get("audio_file_key").and_then(|v| v.as_str())
                else {
                ...
                let transcription_result = match self
                    .storage
                    .get_presigned_url(&audio_file_key, std::time::Duration::from_secs(3600))
                    .await
                {
                    Ok(presigned_url) => {
                        ...
                        let use_direct_upload = std::env::var("TRANSCRIPTION_USE_DIRECT_UPLOAD")
                            ...
                            .unwrap_or_else(|| {
                                ...
                                presigned_url.contains("localhost")
                                    || presigned_url.contains("127.0.0.1")
                            });
                ...
                if pending_records.len() >= BATCH_SIZE {
                    ...
                    let batch_result =
                        execute_speech_transcription_batch_insert(db, &pending_records).await;
...
mod tests {
    ...
    fn test_transform_metadata() {
        ...
        let transform = MicrophoneTranscriptionTransform::new("test-key".to_string(), storage);
