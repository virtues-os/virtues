...
pub async fn run_illustration_job(pool: &SqlitePool) -> Result<()> {
    ...
    let raw_png = generate_image_via_gateway(&full_prompt).await?;
...
async fn find_eligible_day(pool: &SqlitePool) -> Result<Option<EligibleDay>> {
...
async fn pick_top_novelty_subject(pool: &SqlitePool, date: NaiveDate) -> Result<Option<Subject>> {
...
// ── Scene crafter (LLM via Tollbooth) ───────────────────────────────────────
...
async fn craft_scene_prompt(
    ...
    let tollbooth_url =
        std::env::var("TOLLBOOTH_URL").unwrap_or_else(|_| "http://localhost:9002".into());
    ...
    let response = crate::tollbooth::with_system_auth(
        client.post(format!("{tollbooth_url}/v1/chat/completions")),
        &secret,
    )
    ...
    if !response.status().is_success() {
        ...
        return Err(Error::ExternalApi(format!("Scene crafter error {status}: {text}")));
    }
    ...
    let parsed: Resp = response.json().await
        .map_err(|e| Error::ExternalApi(format!("Scene crafter parse error: {e}")))?;
...
/// Call Vercel AI Gateway with Nano Banana (Gemini 2.5 Flash Image) to generate
/// a PNG image. Returns the raw PNG bytes.
///
/// Uses the OpenAI-compatible chat completions endpoint. Nano Banana returns
/// images inline in the response (base64-encoded in content parts).
async fn generate_image_via_gateway(prompt: &str) -> Result<Vec<u8>> {
    let api_key = std::env::var("AI_GATEWAY_API_KEY")
        .map_err(|_| Error::Configuration("AI_GATEWAY_API_KEY not set".into()))?;
    ...
    if !response.status().is_success() {
        let status = response.status().as_u16();
        let text = response.text().await.unwrap_or_default();
        return Err(Error::ExternalApi(format!("Image gen error {status}: {text}")));
    }

    // Parse the response — Vercel AI Gateway returns OpenAI-compat format.
    // For multimodal models, images appear as base64 data in content parts.
    let body: serde_json::Value = response.json().await
        .map_err(|e| Error::ExternalApi(format!("Image gen parse error: {e}")))?;
