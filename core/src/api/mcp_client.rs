...
pub async fn list_mcp_servers(db: &SqlitePool) -> Result<Vec<McpServer>> {
    let rows = sqlx::query(
        r#"
...
pub async fn get_mcp_server(db: &SqlitePool, id: &str) -> Result<McpServerDetail> {
    let row = sqlx::query(
...
pub async fn toggle_mcp_tool(db: &SqlitePool, tool_id: &str) -> Result<bool> {
