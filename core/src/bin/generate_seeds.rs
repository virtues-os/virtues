...
fn main() -> Result<()> {
    ...
    for source in internal_sources {
        ...
        for stream in &source.streams {
            let stream_id =
                deterministic_uuid("stream", &format!("{}:{}", source.name, stream.name));
    ...
    fs::write(&source_connections_path, source_connections_content).with_context(|| {
        format!(
            "Failed to write _generated_source_connections.json to {}",
            source_connections_path.display()
        )
    })?;
    ...
    fs::write(&stream_connections_path, stream_connections_content).with_context(|| {
        format!(
            "Failed to write _generated_stream_connections.json to {}",
            stream_connections_path.display()
        )
    })?;
    ...
    println!("📝 Summary:");
    println!(
        "   - {} source connection(s) generated",
        source_connections_json.connections.len()
    );
    println!(
        "   - {} stream connection(s) generated",
        stream_connections_json.connections.len()
    );
    println!();
    println!("💡 Next steps:");
