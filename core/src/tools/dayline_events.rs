...
async fn create_event(
    ...
    let day_id: Option<String> = sqlx::query_scalar(
        "SELECT id FROM wiki_days WHERE date = ?",
    )
    .bind(&date_str)
    .fetch_optional(pool)
    .await
    ...
    let day_id = match day_id {
        ...
        None => {
            ...
            sqlx::query("INSERT INTO wiki_days (id, date) VALUES (?, ?)")
                .bind(&id)
                .bind(&date_str)
                .execute(pool)
                .await
...
async fn continue_event(
    ...
    let mut query = String::from("UPDATE wiki_events SET end_time = ?, agent_action = 'CONTINUE'");
...
async fn revise_event(
    ...
    let is_user_edited: Option<bool> = sqlx::query_scalar(
        "SELECT CASE WHEN is_user_edited = 1 THEN 1 ELSE 0 END FROM wiki_events WHERE id = ?",
    )
    .bind(event_id)
    .fetch_optional(pool)
    .await
    ...
    if let Some(summary) = event_summary {
        ...
        let event_date = if let Some(et) = end_time.or(start_time) {
            ...
            sqlx::query_scalar::<_, String>(
                "SELECT start_time FROM wiki_events WHERE id = ?"
            )
            .bind(event_id)
            .fetch_optional(pool)
            .await
...
async fn mark_no_data(
    ...
    let prev_unknown: Option<String> = sqlx::query_scalar(
        r#"SELECT e.id FROM wiki_events e
           ...
           WHERE d.date = ? AND e.is_unknown = 1 AND e.end_time = ?
    ...
    if let Some(prev_id) = prev_unknown {
        ...
        sqlx::query("UPDATE wiki_events SET end_time = ?, updated_at = datetime('now') WHERE id = ?")
            .bind(end_time)
            .bind(&prev_id)
            .execute(pool)
            .await
    ...
    // Get or create day
    let day_id: Option<String> = sqlx::query_scalar("SELECT id FROM wiki_days WHERE date = ?")
        .bind(&date_str)
        .fetch_optional(pool)
        .await
        .map_err(|e| ToolError::ExecutionFailed(format!("DB error: {e}")))?;
